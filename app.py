...
def train_model():
    ...
    if os.path.exists(MODEL_PATH):
        ...
        return (
            ...
            bundle["feature_columns"], bundle["median_charge"],
            bundle["metrics"]
        )
    ...
    if using_real_data:
        ...
        df = pd.DataFrame({
            ...
            "TotalCharges": total_chg, "Churn": churn
        })
...
def get_recommendations(data, prob):
    ...
    if data["Contract"] == "Month-to-month":
        recs.append("📋 **Offer a 1-year contract discount** — month-to-month customers churn most.")
    if data["InternetService"] == "Fiber optic":
        recs.append("🌐 **Review Fiber pricing** — Fiber optic users show elevated churn risk.")
    if data["PaymentMethod"] == "Electronic check":
        recs.append("💳 **Encourage auto-pay setup** — electronic check users have higher churn.")
    if data["SeniorCitizen"] == 1:
        recs.append("👴 **Assign a dedicated support agent** — senior customers benefit from personalized care.")
    if data["tenure"] < 12:
        recs.append("🎯 **Enroll in a loyalty rewards program** — first-year customers are high-risk.")
    if data["TechSupport"] == "No":
        recs.append("🛠️ **Offer a free TechSupport trial** — customers without support churn more.")
    if data["OnlineSecurity"] == "No":
        recs.append("🔒 **Bundle OnlineSecurity** — security add-ons improve retention.")
    if not recs:
...
def run_batch_prediction(uploaded_df, model, scaler, feature_columns, median_charge):
    ...
    for _, row in uploaded_df.iterrows():
        try:
            ...
            results.append({
                **{k: v for k, v in data.items() if k in ["tenure", "Contract", "MonthlyCharges"]},
                "Churn Probability": f"{prob:.2%}",
                "Risk Level": "🔴 High" if prob >= 0.5 else ("🟡 Medium" if prob >= 0.3 else "🟢 Low")
            })
        except Exception:
...
def page_predict(model, scaler, feature_columns, median_charge):
    ...
    with col3:
        ...
        st.info(f"📊 Estimated Total Charges: **${total:,.2f}** (Monthly × Tenure)")
    ...
    if uploaded:
        ...
        if st.button("🔮 Run Batch Prediction", type="primary"):
            with st.spinner("Predicting..."):
                results = run_batch_prediction(df, model, scaler, feature_columns, median_charge)
